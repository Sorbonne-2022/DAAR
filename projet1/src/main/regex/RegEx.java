...
public class RegEx {
    ...
    private NDFAutomaton toNDFAutomaton(RegExTree ret) {
        ...
        if (ret.root == CONCAT) {
            //IMPLICIT REPRESENTATION HERE: INIT STATE IS ALWAYS 0; FINAL STATE IS ALWAYS transitionTable.length-1
            NDFAutomaton gauche = toNDFAutomaton(ret.subTrees.get(0));
            int[][] tTab_g = gauche.transitionTable;
            ArrayList<Integer>[] eTab_g = gauche.epsilonTransitionTable;
            NDFAutomaton droite = toNDFAutomaton(ret.subTrees.get(1));
        ...
        if (ret.root == ETOILE) {
            //IMPLICIT REPRESENTATION HERE: INIT STATE IS ALWAYS 0; FINAL STATE IS ALWAYS transitionTable.length-1
            NDFAutomaton fils = toNDFAutomaton(ret.subTrees.get(0));
